...
SYNAPSE_WEIGHT_CHANGE_CHANCE = 0.00
SYNAPSE_SWITCH_CHANCE = 0.00
...
class NetworkGenome:
    ...
    def mutate(self):
        # synapse activation/deactivation and synapse weight change
        for sg in self.synapse_gene:
            synapse_switch = random.choices([True, False], [SYNAPSE_SWITCH_CHANCE, 1 - SYNAPSE_SWITCH_CHANCE])[0]
            synapse_weight_change = random.choices([True, False], [SYNAPSE_WEIGHT_CHANGE_CHANCE, 1 - SYNAPSE_WEIGHT_CHANGE_CHANCE])[0]
            if synapse_switch:
                print(f'[synapse en/disable mutation] @ sg {sg.id}')
                sg.is_on = not sg.is_on
            if synapse_weight_change:
                print(f'[synapse weight change mutation] @ sg {sg.id}')
                sg.weight = random.uniform(0, 1)


        # neuron bias change
        for ng in self.all_neuron_genes:
            neuron_bias_change = random.choices([True, False], [NEURON_BIAS_CHANGE_CHANCE, 1 - NEURON_BIAS_CHANGE_CHANCE])[0]
            if neuron_bias_change:
                print(f'[neuron bias change mutation] @ ng {ng.id}')
                ng.bias = random.uniform(0, 1)

        # neuron add
        # done by disabling an existing synapse, adding two new synapses, with a neuron in between
        neuron_add = random.choices([True, False], [NEURON_ADD_CHANCE, 1 - NEURON_ADD_CHANCE])[0]
        ...
        if neuron_add:
            ...
            # make a new neuron
            new_neuron = NeuronGene(len(self.all_neuron_genes) + 1, 0)
            self.neuron_gene.append(new_neuron)
            self.all_neuron_genes.append(new_neuron)

            print(f'[neuron addition mutation] @ sg {to_disable.id}')

            # create two new synapses, and connect the new neuron with it
            new_synapse_into = SynapseGene(1, to_disable.outof, new_neuron, 1, True)
            new_synapse_outof = SynapseGene(2, new_neuron, to_disable.into, to_disable.weight, True)

            self.synapse_gene.append(new_synapse_into)
            self.synapse_gene.append(new_synapse_outof)

        # synapse add
        # must ensure new synapse does not make a loop
        
        synapse_add = random.choices([True, False], [SYNAPSE_ADD_CHANCE, 1 - SYNAPSE_ADD_CHANCE])[0]
        ...
        if synapse_add:
            ...
            # print(sorted_neurons)
            # outof_candidates = list(range(len(self.hidden_output_neurons) - 1))
            # print(outof_candidates)
